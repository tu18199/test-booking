<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="list" border fit highlight-current-row stripe style="width: 100%">
        ...
        <div class="permissions-container">
          <div class="block">
            <el-form :model="currentUser" label-width="80px" label-position="top">
              <el-form-item label="Phân quyền menu">
    ...
    <el-dialog :title="newUser.title" :visible.sync="dialogFormVisible" :destroy-on-close="true">
      <div v-loading="userCreating" class="form-container">
        <el-form ref="userForm" :rules="newUser.isEdit ? rulesEdit : rules" :model="newUser" label-position="left" label-width="150px" style="max-width: 500px;">
          ...
          <el-form-item :label="$t('user.email')" prop="email">
            <el-input v-model="newUser.email" :disabled="newUser.isEdit" />
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.resetNewUser();
      this.newUser.title = 'Tạo mới tài khoản';
    ...
    handleUpdate(row) {
      this.resetNewUser();
      this.newUser = { ...row };
      this.newUser.role = this.newUser.roles[0];
      this.newUser.isEdit = true;
      this.newUser.title = 'Chỉnh sửa tài khoản';
      this.dialogFormVisible = true;
      this.$nextTick(() => {
        this.$refs['userForm'].clearValidate();
      });
    ...
    createUser() {
      this.$refs['userForm'].validate((valid) => {
        if (valid) {
          ...
          const type = this.newUser.isEdit ? 'Chỉnh sửa ' : 'Tạo mới ';
          userResource
            .store(this.newUser)
            .then(response => {
              this.$message({
                message: type + this.newUser.name + '(' + this.newUser.email + ') thành công.',
                type: 'success',
                duration: 3 * 1000,
              });
              this.resetNewUser();
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['Tài khoản', 'Tên', 'Email', 'Phân quyền'];
        const filterVal = ['email', 'name', 'email', 'role'];
